...
import { addToCart, removeCartItem, decreaseCartItem, clearCart, getTotals } from "@/src/@core/redux/feautures/cartSlice";
      ...
      {cart.cartItem.length === 0 ? (
        ...
        <div>
                ...
                <div className={style["cart-item"]} key={cartItem.id}>
                  <div className={style["cart-product"]}>
                    ...
                    <div  className={style["cart-detail"]} >
                      ...
                      <Button onClick={()=> handleRemoveFromCart(cartItem)} className={style["button"]}>
                        Remove
                      </Button>
                    </div>
                  </div>
                  <div className={style["cart-product-price"]}>${cartItem.price}</div>
                  <div className={style["cart-product-quantity"]} >
                    <Button onClick={() => handleDecreaseCart(cartItem)} className={style["button"]}>
                      -
                    </Button>
                    <div  className={style["count"]}>{cartItem.cartQuantity}</div>
                    <Button className={style["button"]} onClick={() => handleAddToCart(cartItem)}>+</Button>
                  </div>
          ...
          <Pane className={style["cart-summary"]}>
                          <Button onClick={() => handleClearCart()} className={style["clear-btn"]}>
              Clear Cart
            </Button>
            <Pane className={style["cart-checkout"]}>
              ...
              <Text className={style["p"]}>Taxes and shipping calculated at checkout</Text>
